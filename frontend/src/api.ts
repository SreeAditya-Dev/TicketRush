...
export const getSeats = async (eventId?: string, date?: string, time?: string): Promise<Seat[]> => {
...
export const bookSeat = async (
  ...
  try {
    const response = await api.post("/book-seat", { seatCode, userId, strategy, eventId, date, time });
